...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.augmentor = Augmentor(
            ...
            augment_dict=hyp['soda']['augment_dict']['train'] if 'train' in prefix else hyp['soda']['augment_dict'][
                'val']
        )
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(img), torch.from_numpy(ori_img), labels_out, self.img_files[index], shapes
    ...
    def collate_fn(batch):
        img, ori_img, label, path, shapes = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(img, 0), torch.stack(ori_img, 0), torch.cat(label, 0), path, shapes
